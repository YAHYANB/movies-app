...
function Header({search, setSearch}) {
  return (
    <nav className="border-b border-gray-800">
      <div className="container mx-auto  md:flex-row flex flex-col items-center justify-between px-4 py-4 text-sm">
        <ul className="flex flex-col md:flex-row items-center">
          ...
          <li className="md:ml-8 mt-4 md:mt-0">
            <Link to="" className="hover:text-gray-300">Actors</Link>
          </li>
        </ul>
        <div className="flex flex-col md:flex-row items-center">
          <div className="relative mt-4 md:mt-0">
            <input type="text" value={search} onChange={(e)=>setSearch(e.target.value)} className="bg-gray-800 text-sm rounded-full w-64 px-4 pl-9 py-1 focus:outline-none focus:shadow-outline" placeholder="Search" />
            <div className="absolute top-0 ">
              <svg className="ml-1.5 mt-1 h-5 w-5 text-with-500"  width="20" height="20" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round">  <path stroke="none" d="M0 0h24v24H0z"/>  <circle cx="10" cy="10" r="7" />  <line x1="21" y1="21" x2="15" y2="15" /></svg>
            </div>
          </div>
          <div className="ml-4 mt-4 md:mt-0">
            <img src={Img} alt="avatar" className="rounded-full w-8 h-8" />
          </div>
        </div>
      </div>
    </nav>
